...
- Multi-language support with locale-based translation files: `src/lang/es-es.json`, `src/lang/en-us.json`, `src/lang/pt-pt.json`, `src/lang/zh-cn.json`, and `src/lang/ko-kr.json`.
...
- Payload catalog loading now accepts JSON format only (`PS4` and/or `PS5` sections); legacy text catalogs are no longer supported.
